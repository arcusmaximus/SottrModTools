...
namespace TrRebootTools.Extractor.Controls
{
        ...
        where TFile : class
    {
        ...
        private void OnGetNodeCellCellImage(VirtualTreeView tree, VirtualTreeNode treeNode, int column, out Image image)
        {
            FileTreeNode fileNode = _tvFiles.GetNodeData<FileTreeNode>(treeNode);
            image = fileNode?.Image;
        }

        private void GetCellText(VirtualTreeView tree, VirtualTreeNode treeNode, int column, out string celltext)
        {
            FileTreeNode fileNode = _tvFiles.GetNodeData<FileTreeNode>(treeNode);
            if (fileNode == null)
            {
