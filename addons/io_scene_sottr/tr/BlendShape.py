...
from io_scene_sottr.util.CStruct import CInt, CLong, CStruct
...
class BlendShape(SlotsBase):
    ...
    @staticmethod
    def read(reader: BinaryReader, model_header: ModelDataHeader, num_vertices: int) -> list[Optional["BlendShape"]]:
        header = reader.read_struct(_BlendShapesHeader)
        reader.align(0x20)

        supported_blendshape_bitmasks: Sequence[int] = reader.read_uint32_list((model_header.num_blend_shapes + 0x1F) >> 5)
        reader.align(0x20)
        ...
        for _ in range(header.num_blend_shapes):
            blend_shapes.append(BlendShape())
    ...
    @staticmethod
    def write(writer: BinaryWriter, blend_shapes: list[Optional["BlendShape"]], num_vertices: int) -> None:
