...
from io_scene_sottr.tr.Cloth import Cloth, CollisionKey, ClothMass, ClothMassAnchorBone, ClothSpring, ClothStrip
...
class ClothExporter:
        ...
        with BlenderHelper.enter_edit_mode(bl_armature_obj):
            for bl_bone in cast(bpy.types.Armature, bl_armature_obj.data).edit_bones:
                ...
                if bone_id_set.local_id is not None:
                    result[bone_id_set.local_id] = _BoneInfo(bone_id_set.global_id, Vector(bl_bone.head) / self.scale_factor)
    ...
    def export_cloth(
        ...
        collision_bounding_boxes: dict[CollisionKey, _BoundingBox]
    ) -> None:
        ...
        component_writer = ResourceBuilder(ResourceKey(ResourceType.DTP, cloth_id_set.component_id))
        tr_cloth.write(definition_writer, component_writer)

        definition_file_path = os.path.join(folder_path, Collection.make_resource_file_name(definition_writer.resource))
        with open(definition_file_path, "wb") as definition_file:
            definition_file.write(definition_writer.build())
        
        component_file_path = os.path.join(folder_path, Collection.make_resource_file_name(component_writer.resource))
        with open(component_file_path, "wb") as component_file:
            component_file.write(component_writer.build())
    ...
    def add_cloth_strip_masses(
        ...
        for bl_vertex in bl_cloth_strip_mesh.vertices:
            ...
            bl_bone = cast(bpy.types.Bone | None, bl_armature.bones.get(bone_name))
            if bl_bone is None:
    ...
    def add_cloth_strip_collisions(self, tr_cloth_strip: ClothStrip, bl_cloth_strip_obj: bpy.types.Object, collision_bounding_boxes: dict[CollisionKey, _BoundingBox]) -> None:
        ...
        for collision_key, collision_bounding_box in collision_bounding_boxes.items():
            if cloth_strip_bounding_box.intersects(collision_bounding_box):
                tr_cloth_strip.collision_keys.append(collision_key)

    def get_world_bounding_box(self, bl_obj: bpy.types.Object) -> _BoundingBox:
        corners = Enumerable(bl_obj.bound_box).select(lambda c: cast(Vector, cast(Matrix, bl_obj.matrix_world) @ Vector(c))).to_list()
        min_corner = Vector((
