...
class CStructTypeMapping(ICStructTypeMapping, typing.Generic[TMappedType, TCType]):
    mapped_type: typing.Any
    c_type:      typing.Any
...
_type_mappings: dict[tuple[typing.Any, Bitness], ICStructTypeMapping] = {}
...
class CArray(typing.Generic[TArrayItem, TArrayLength]):
    ...
    def __class_getitem__(cls, params: tuple[typing.Any, typing.Any]) -> typing.Any:
        item_type: typing.TypeVar | type = params[0]
        if isinstance(item_type, typing.TypeVar):
            return super().__class_getitem__(params)        # type: ignore

        length: int = params[1].__args__[0]
        array_alias = CArrayAlias(item_type, length)

        for bitness in (Bitness.X86, Bitness.X64):
            if (item_type, bitness) not in _type_mappings:
                return create_c_array_type(item_type, length)

        for bitness in (Bitness.X86, Bitness.X64):
            if (array_alias, bitness) in _type_mappings:
                continue

            item_type_mapping = _type_mappings[(item_type, bitness)]
            _type_mappings[(array_alias, bitness)] = CArrayTypeMapping[item_type_mapping.mapped_type, item_type_mapping.c_type, typing.Literal[length]](  # type: ignore
                array_alias,
...
class CArrayTypeMapping(ICStructTypeMapping, typing.Generic[TMappedItemType, TCItemType, TArrayLength]):
    mapped_type: typing.Any
    c_type: typing.Any
...
class CStructMeta(type(ctypes.Structure)):
    def __new__(cls: type, class_name: str, bases: tuple[type, ...], namespace: dict[str, typing.Any]) -> type:
        if class_name not in ("CStruct", "CStruct32", "CStruct64"):
            ...
            bitness = cast(type[CStruct], Enumerable(bases).first(lambda b: issubclass(b, CStruct)))._bitness       # type: ignore
            ...
            for field_name, field_type in typing.cast(dict[str, typing.Any], namespace["__annotations__"]).items():
                ...
                field_type_mapping = _type_mappings.get((field_type, bitness))
...
class CStruct(ctypes.Structure, struct_secondary_base, metaclass=struct_metaclass):         # type: ignore
    _bitness: typing.ClassVar[Bitness] = Bitness.ANY

    _field_infos_: dict[str, CStructField] = {}

    @classmethod
    def register_type_mappings(cls, *mappings: ICStructTypeMapping) -> None:
        for mapping in mappings:
            _type_mappings[(mapping.mapped_type, cls._bitness)] = mapping
            if cls._bitness == Bitness.ANY:
                _type_mappings[(mapping.mapped_type, Bitness.X86)] = mapping
                _type_mappings[(mapping.mapped_type, Bitness.X64)] = mapping
...
class CStruct32(CStruct):
    _bitness = Bitness.X86

class CStruct64(CStruct):
    _bitness = Bitness.X64
